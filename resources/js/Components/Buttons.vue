...
const props = defineProps(['type', 'icon', 'shadow', 'disabled', 'size', 'iconPosition','color','class'])
...
<template>
    <button :type="props.type" :class="[
        ...
        props.disabled && 'disabled:cursor-not-allowed disabled:hover:shadow-none',
        icon && `inline-flex items-center justify-center gap-2 ${props.size === 'sm' ? 'gap-2' : 'gap-3'}`,
        getSize(),
        getColor(),
        props.class,
    ]">
