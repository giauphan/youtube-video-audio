        ...
        <TabGroup>
            <TabList class="my-10 flex gap-3">
                <Tab as="template" class="focus-visible::border-none" v-slot="{ selected }">
                    <Buttons :class="['focus-visible::border-0 p-1 rounded', selected ? 'bg-white text-black' : 'bg-gray-500 text-white']">
                        Video
                    </Buttons>
                </Tab>
                <Tab v-slot="{ selected }">
            ...
            <TabPanels>
                <TabPanel>
                    {{ console.log(video_short) }}
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 min-h-[300px]">
                        <a :href="route('video.index', ('video', video.video_id))" v-if="videos" v-for="video in videos.data">
                ...
                <TabPanel>
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 min-h-[300px]">
                        <a :href="route('video.index', ('video', video.video_id))" v-if="video_short" v-for="video in video_short.data">
                            <img :src="video.thumbnail ?? null" class="mb-5 aspect-video w-full h-56 rounded-lg"
                                :alt="truncateTitle(video.title)" />
                            <h1 class="text-xl text-white font-bold bg-black overflow-hidden" style="display: -webkit-box;
                    -webkit-box-orient: vertical;
